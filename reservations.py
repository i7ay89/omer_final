...
def check_availability(cursor, party_size, time, smoking=False, outside=False, bar=False, handicapped=False):
    cursor.execute('''SELECT ID FROM tables WHERE smoking={} AND outdoor={} AND
                      bar={} AND handicapped={} AND capacity<={} + 2 AND capacity >= {}'''.format(bool_to_str(smoking),
                                                                                                  ...
                                                                                                  party_size,
                                                                                                  party_size))
    time = str(datetime.strptime(time, "%Y-%m-%dT%H:%M"))
    results = cursor.fetchall()
    if isinstance(results, type([])):
        matching_tables = [tup[0] for tup in results]
    else:
        matching_tables = results[0]
...
def filter_reservations(cursor, table_id=None, date=None, party=None, name=None):
    query = """SELECT * FROM reservations WHERE 1=1 """
    if table_id:
        query += """AND table_id={} """.format(table_id)
    if date:
        from_time = str(datetime.strptime(date, "%Y-%m-%dT%H:%M") + timedelta(hours=0))
        until_time = str(datetime.strptime(date, "%Y-%m-%dT%H:%M") + timedelta(hours=12))
...
def create_reservation(cursor, party_size, time, table_id, name, phone):
    from_time = str(datetime.strptime(time, "%Y-%m-%dT%H:%M") + timedelta(hours=0))
    until_time = str(datetime.strptime(time, "%Y-%m-%dT%H:%M") + timedelta(hours=2))
    try:
